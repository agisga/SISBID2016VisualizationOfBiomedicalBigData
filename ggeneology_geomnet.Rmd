...
hw <- plyr::rbind.fill(data.frame(advisorMGID=145799, 
                              advisorName="Hadley Alexander Wickham"), hw)
names(hw)[2:3] <- c("parent", "child")
...
```{r}
statisticians <- c("Thomas Lumley", "Rob Hyndman", "Grace Wahba", 
                   "Gordon Smyth", "Elizaveta Levina", "David Cox", 
                   "Irène Gijbels", "Gerda Claeskens")
plotDegMatrix(statisticians, statIG, statGeneal, 
              "Statistician", "Statistician", "Degree") +
  ggplot2::scale_fill_continuous(low = "white", high = "darkgreen") +
  ggplot2::theme(legend.title = ggplot2::element_text(size = 15),
                 legend.text = ggplot2::element_text(size = 15))
```
...
p + geom_net(label = TRUE, size = 15, labelcolour="white", vjust=0.5, 
             hjust=0.5, directed=TRUE, arrowgap=0.075) + 
  theme_net()
```
...
```{r}
p + geom_net(label = TRUE, labelcolour="white", vjust=0.5, 
             hjust=0.5, directed=TRUE, arrowgap=0.075, 
             aes(size = Predominance)) + 
  theme_net() + facet_wrap(~Ethnicity) + scale_size(range=c(5,15)) + 
...
```{r}
ggplot(data = blood$vertices) +
  geom_bar(aes(x = Ethnicity, weight = Predominance)) +
